...
export default function GenerateButton({ formData }) {
  ...
  return (
    ...
    >
      <div
        className={`absolute z-10 ${allValues ? "text-lg leading-[14px] animate-grow-text" : "text-xs leading-[10px]"} text-white transition-all duration-500`}
      >
        <div>gen</div>
        <div>er</div>ate
      </div>
      ...
      ></div>
      <div
        className={`penta animate-rotate relative h-12 w-12 bg-green-500 ${allValues && "h-16 w-16 animate-grow"} transition-all duration-500`}
      ></div>
    </button>
  );
}
